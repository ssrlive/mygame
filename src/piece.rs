...
pub fn rotate_piece(
    ...
    if keyboard_input.just_pressed(KeyCode::ArrowUp) {
        ...
        let original_blocks: Vec<Block> = q_piece.iter().map(|(_, block, _)| *block).collect();
        ...
        if whether_colliding(&q_piece, &q_board) {
            for (_, mut block, mut transform) in &mut q_piece {
                *block = shift_block(*block, Some(-1), None);
                transform.translation = block.translation();
            }
        }
        if whether_colliding(&q_piece, &q_board) {
        ...
        if whether_colliding(&q_piece, &q_board) {
            for (_, mut block, mut transform) in &mut q_piece {
                *block = shift_block(*block, Some(3), None);
                transform.translation = block.translation();
            }
        }
        if whether_colliding(&q_piece, &q_board) {
        ...
        // 恢复旋转前样子
        if whether_colliding(&q_piece, &q_board) {
            for (index, (_, mut block, mut transform)) in (&mut q_piece).into_iter().enumerate() {
...
pub fn auto_generate_new_piece(
    ...
    if query.is_empty() {
        ...
        let piece_type = piece_config.piece_type;
...
pub fn random_7_pieces() -> Vec<PieceConfig> {
    ...
    loop {
        match rng.random_range(0..PieceType::PIECE_AMOUNT) {
            0 => piece_type_set.insert(PieceType::I),
            1 => piece_type_set.insert(PieceType::J),
            2 => piece_type_set.insert(PieceType::L),
            3 => piece_type_set.insert(PieceType::O),
            4 => piece_type_set.insert(PieceType::S),
            5 => piece_type_set.insert(PieceType::T),
            6 => piece_type_set.insert(PieceType::Z),
            _ => panic!("Random value is unexpected"),
        };
