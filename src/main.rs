...
fn setup_screen(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands.spawn(Sprite::from_image(asset_server.load("screen.png")));
    commands
        .spawn(init_text(
            "000000",
            TEXT_SCORE_X,
...
fn scoreboard_system(
    ...
    if game.deleted_lines > 0 {
        ...
        if let Ok(mut text) = query.p1().get_single_mut() {
            text.0 = format!("{:06}", game.lines);
        }
        if let Ok(mut text) = query.p2().get_single_mut() {
            text.0 = format!("{:02}", game.level);
        }
    }
...
#[inline]
fn sprit_bundle(width: f32, color: Color, trans: Vec2) -> (Sprite, Transform) {
    (
        Sprite::from_color(color, Vec2::new(width, width)),
        Transform::from_translation(Vec3::new(trans.x, trans.y, 0.1)),
    )
}
